    ...
    for index, pokemon in pokemon_index_to_pokemon.items():
        ...
        if pokemon.pre_evolution_index_to_level_to_moves is not None:
            for moves_list in [
                ...
                for i, level_to_moves in pokemon.pre_evolution_index_to_level_to_moves
            ]:
                attacks += moves_list

        if pokemon.move_tutor_attacks is not None:
            attacks.append(pokemon.move_tutor_attacks)

        if pokemon.game_to_level_to_moves is not None:
            for attack_level, moves_list in [
        ...
        if pokemon.form_to_level_up_attacks is not None:
            for moves_list in [
                level_to_moves.values
                for i, level_to_moves in pokemon.form_to_level_up_attacks.items()
            ]:
                attacks += moves_list
